...
const Document = () => {
    ...
    useEffect(() => {
        ...
        const getDokumen = async () => {
            try {
                const response = await axiosPrivate.get('/pdfgen', {
    ...
    const createAndDownloadPdf = async (e) => {
        ...
        axios.post('/pdfgen',
            ...
            .then(() => axios.get('/pdfgen', { responseType: 'blob' }))
            .then((res) => {
    ...
    const getDokumen = async () => {
        try {
            ...
            console.error(err);
            navigate('/document8', { state: { from: location }, replace: true });
        }
    }
